...
at once, including a task that runs an integration test. Normally an integration test
can be a script that interact with your app through a browser in an automated way,
testing some conditions are met when the webapp is loaded, and can also perform some
actions, all can be written in Javascript with a framework like Playwright or Selenium,
or an integration test can be a shell script maybe written in Python that make calls
to your API, then check the result and continues making more calls based on the results.
In the example above the integration tests were written with Playwright to test the
webapp in a headless browser (service `web-ci-tests`), so they were written and stored
in the same image used to store the webapp (service `web`), but executed with
a different command:
...
There are 3 images, all starting with the prefix "mrsarm/" that
are used for our services _api_, _api-worker_ and _web_ (and
the CI task _web-ci-tests_), while the other images
_postgres:15_ and _rabbitmq:3_ are DB services
...
a new feature in the API to add a new feature, and that feature
...
GitHub Actions set the environment variable `GITHUB_REF` with
the name of the branch the CI task is running against to, so making
the change above, `docker compose run` will replace at runtime
the expression `mrsarm/web:${GITHUB_REF:-latest}` with
`mrsarm/web:client-vat-field`, producing the expected result of
executing the tests on the image desired, while when running
the same compose file locally, the expression will be turned
into `mrsarm/web:latest` because the env variable `GITHUB_REF`
doesn't exist. All right, right?

The problem is, when you move forward with the rest of the apps where
you add the feature, how you distinguish between the services that already
have the feature developed and published in the docker registry and
which ones don't? if you add the suffix `${GITHUB_REF:-latest}` to all the
...
if the tag exists in the registry, otherwise keep using the same `latest` set
in the compose file, or whatever tag is set. You can achive this with the command
`pose config`, that like `docker compose config` it outputs a new compose file
but pre-processing it according to the arguments received. The following
will output a new `ci.yaml` file with the desired output, while showing in the
terminal (or the CI logs) what is doing while fetching the information from
the docker registry:
...
In the output we can see the images found with the tag are `mrsarm/api` and
`mrsarm/web`, while the others like `mrsarm/api-worker` don't, so the resulting
services definition in the new `ci.yaml` will be like:
...
There are more options that you can see with `pose config --help`.

#### Threads

The process of checking all the images your compose file has can
be slow, specially in big app composed of dozens of apps.
There are two special arguments that can help the process to speed up,
one is `--threads NUM` that allows to specify the max number of parallel
...
`FILTER` can be an expression like `regex=NAME` (`=` filter ‒ in) or `regex!=EXPR`
(`!=` filter ‒ out), where `EXPR` is a regex expression. In our example, all the apps we
...
The resulting `ci.yaml` will be identical than not using the filter, because it's unlikely
and even undesired to have an official Postgres image `postgres:client-vat-field`, but more
importantly, the execution in our CI pipeline will be much faster.
...
`regex!=` could be as follows for the same example: `postgres|rabbitmq`. Because
it's an "exclusion" expression, all images with the string `postgres` or `rabbitmq`
